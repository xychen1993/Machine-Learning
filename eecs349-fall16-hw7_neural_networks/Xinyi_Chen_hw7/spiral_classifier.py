...
def main():
    ...
    n_classes = len(set(classification))

    plot_spiral(position,classification,'start.png')
    if do_linear == 0:
        model = linear_classifier(position,classification,n_classes)
        plot_spiral_and_predicted_class(position, classification, model, 'linear.png', 'linear')
    
    else:
        model2 = non_linear_classifier(position,classification,n_classes)
